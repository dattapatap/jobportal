        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="col-md-6 offset-3">
                    <form class="card" action="{{ route('admin.audit.update', $audit->id ) }}" method="POST" accept-charset="UTF-8">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row">
                                        ...
                                        <select name="country" id="country" class="form-control select2">
                                            @if(isset($countries))
                                                @foreach ($countries as $item)
                                                        <option value="{{ $item->id }}" @if( old('country',$audit->country)==$item->id ) selected @endif > {{ $item->name  }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                        @error('country')
                                ...
                                <div class="col-md-12">
                                    <div class="form-group pb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ old('name', $audit->authority ) }}" name="name"  placeholder="Audit Name">
                                        @error('name')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
